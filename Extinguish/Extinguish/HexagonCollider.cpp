#include "HexagonCollider.h"
#include "SphereCollider.h"
#include "CapsuleCollider.h"
#include "BoxCollider.h"
#include "Physics.h"
#include "GameObject.h"
//#include "FloorController.h"
#include "EventDispatcher.h"
#include "PulseFloorEvent.h"
#include "MathStuff.h"

HexagonCollider::HexagonCollider(GameObject* o, float d, float _height, ED2Mesh* colMesh) : Collider(o, false, CTHex)
{
	mesh = colMesh;
	poses = nullptr;
	hex.d = d;
	hex.s = d * 0.5f;
	hex.seg.m_Start = o->GetTransform()->GetPosition();
	hex.seg.m_End = o->GetTransform()->GetPosition() + float3(0, _height, 0);
	height = _height;
	hex.h = sqrtf(3)*hex.s;
	row = 0;
	col = 0;
}

HexagonCollider::HexagonCollider(int _row, int _col, float3* positions, float _height, float d, GameObject* o, FloorController* FC, ED2Mesh* colMesh) : Collider(o, false, CTHex)
{
	mesh = colMesh;
	poses = positions;
	height = _height;
	hex.d = d;
	hex.s = d * 0.5f;
	hex.h = sqrtf(3)*hex.s;
	row = _row;
	col = _col;
	floorController = FC;
	/*unsigned int* colors = floorController->getColors();
	originalColor = new unsigned int[row * col];
	int size = row * col;
	for (int i = 0; i < size; ++i)
	{
		originalColor[i] = colors[i];
	}*/
}

Hexagon* HexagonCollider::GetWorldHex()
{
	hex.seg.m_Start = GetGameObject()->GetTransform()->GetPosition();
	hex.seg.m_End = GetGameObject()->GetTransform()->GetPosition() + float3(0, height, 0);
	return &hex;
}

Hexagon* HexagonCollider::GetWorldHex(int i)
{
	hex.seg.m_Start = poses[i];
	hex.seg.m_End = poses[i] + float3(0, height, 0);
	return &hex;
}

AABB checksphereaabb;
AABB sphcheckaabb;
bool HexagonCollider::CheckSphereAABB(int _min, int _max, Sphere s)
{
	checksphereaabb.max = poses[_max] + float3(1, 20, 1);
	checksphereaabb.min = poses[_min] - float3(1, 10, 1);
	sphcheckaabb.min = s.m_Center - float3(s.m_Radius, s.m_Radius, s.m_Radius);
	sphcheckaabb.max = s.m_Center + float3(s.m_Radius, s.m_Radius, s.m_Radius);
	return AABBtoAABB(sphcheckaabb, checksphereaabb);
}

bool HexagonCollider::CheckSphereAABB(AABB test, Sphere s)
{
	AABB cap;
	cap.min = s.m_Center - float3(s.m_Radius, s.m_Radius, s.m_Radius);
	cap.max = s.m_Center + float3(s.m_Radius, s.m_Radius, s.m_Radius);
	return AABBtoAABB(test, cap);
}

AABB checkCapaabb;
AABB capcheckaabb;
bool HexagonCollider::CheckCapsuleAABB(int _min, int _max, Capsule s)
{
	checkCapaabb.max = poses[_max] + float3(1, 20, 1);
	checkCapaabb.min = poses[_min] - float3(1, 10, 1);
	capcheckaabb.min = s.m_Segment.m_Start - float3(s.m_Radius, s.m_Radius, s.m_Radius);
	capcheckaabb.max = s.m_Segment.m_End + float3(s.m_Radius, s.m_Radius, s.m_Radius);
	return AABBtoAABB(checkCapaabb, capcheckaabb);
}

bool HexagonCollider::CheckCapsuleAABB(AABB test, Capsule s)
{
	AABB cap;
	cap.min = s.m_Segment.m_Start - float3(s.m_Radius, s.m_Radius, s.m_Radius);
	cap.max = s.m_Segment.m_End + float3(s.m_Radius, s.m_Radius, s.m_Radius);
	return AABBtoAABB(test, cap);
}

bool HexagonCollider::CheckFloor2Capsule(CapsuleCollider* cap, int f, float dt)
{
	GameObject* ob = objects[f];
	Capsule c = cap->GetWorldCapsule();
	float3 pastPos = ob->GetComponent<Physics>()->pastPos + cap->GetCapsule().m_Segment.m_Start;
	GameObject* tg = GetGameObject();
	Transform* tgt = tg->GetTransform();
	float smallestT = 100;
	bool collided = false;
	//Top
	if (CheckCapsuleAABB(int_floor(row * 0.49) * col, row * col - 1, c))
	{
		//TopTop
		if (CheckCapsuleAABB(int_floor(row * 0.741) * col, row * col - 1, c))
		{
			//TopTopTop
			if (CheckCapsuleAABB(int_floor(row * 0.821) * col, row * col - 1, c))
			{
				if (CheckCapsule(tg, ob, int_floor(row*0.821), row, cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
			//TopTopBottom
			if (CheckCapsuleAABB(int_floor(row * 0.741) * col, int_ceil(row * 0.839) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, int_floor(row * 0.741), int_ceil(row * 0.839), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
		}
		//TopBottom
		if (CheckCapsuleAABB(int_floor(row * 0.48) * col, int_ceil(row * 0.759) * col + col - 1, c))
		{
			//TopBottomTop
			if (CheckCapsuleAABB(int_floor(row * 0.621) * col, int_ceil(row * 0.759) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, floor(row * 0.621), int_ceil(row * 0.759), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
			//TopBottomBottom
			if (CheckCapsuleAABB(int_floor(row * 0.48) * col, int_ceil(row * 0.639) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, int_floor(row * 0.48), int_ceil(row * 0.639), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
		}
	}
	//Bottom
	if (CheckCapsuleAABB(0, (int)(ceil(row * 0.51)) * col + col - 1, c))
	{
		//BottomTop
		if (CheckCapsuleAABB(int_floor(row * 0.241) * col, int_ceil(row * 0.51) * col + col - 1, c))
		{
			//BottomTopTop
			if (CheckCapsuleAABB(int_floor(row * 0.371) * col, int_ceil(row * 0.51) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, int_floor(row * 0.371), int_ceil(row * 0.51), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
			//BottomTopBottom
			if (CheckCapsuleAABB(int_floor(row * 0.241) * col, int_ceil(row * 0.389) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, int_floor(row * 0.241), int_ceil(row * 0.389), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
		}
		//BottomBottom
		if (CheckCapsuleAABB(0, (int)(ceil(row * 0.259)) * col + col - 1, c))
		{
			//BottomBottomTop
			if (CheckCapsuleAABB(int_floor(row * 0.121) * col, int_ceil(row * 0.259) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, int_floor(row * 0.121), int_ceil(row * 0.259), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
			//BottomBottomBottom
			if (CheckCapsuleAABB(0, (int)ceil(row * 0.121) * col + col - 1, c))
			{
				if (CheckCapsule(tg, ob, 0, (int)ceil(row * 0.121), cap, c, pastPos, f, dt, smallestT))
					collided = true;
			}
		}
	}

	if (!collided)
	{
		if (CollidingWith[f])
		{
			tg->OnCollisionExit(cap);
			ob->OnCollisionExit(this);
			CollidingWith[f] = false;
		}
	}
	return collided;
}

bool HexagonCollider::CheckCapsule(GameObject* tg, GameObject* ob, int _min, int _max, CapsuleCollider* cap, Capsule c, float3 pastPos, int f, float dt, float& St)
{
	AABB test;
	AABB capBox;
	capBox.max = c.m_Segment.m_End + c.m_Radius;
	capBox.min = c.m_Segment.m_Start - c.m_Radius;
	bool collided = false;
	float smallT = St;
	for (int i = _min; i < _max; ++i)
	{
		test.min = poses[i * col] - float3(2, 2, 2);
		test.max = poses[i * col + col - 1] + float3(2, 22, 2);
		if (AABBtoAABB(test, capBox))
		{
			for (int j = 0; j < col; ++j)
			{
				float3 n = HexagonToCapsule(*GetWorldHex(i * col + j), c, pastPos, smallT, mesh);
				if (!n.isEqual(zeroF))
				{
					collisionNormal = n;
					cap->collisionNormal = n;
					bool apg = false;
					if (!n.isEqual(float3(0, 1, 0)))
						apg = true;
					Physics* op = ob->GetComponent<Physics>();
					if (op)
					{
						op->HandlePhysics(ob->GetTransform(), ob->GetTransform()->GetVelocity(), c.m_Segment.m_Start - cap->GetCapsule().m_Segment.m_Start, false, n, apg, true);
						if (!CollidingWith[f])
						{
							tg->OnCollisionEnter(cap);
							ob->OnCollisionEnter(this);
							CollidingWith[f] = true;
						}
						else if (CollidingWith[f])
						{
							tg->OnCollisionStay(cap);
							ob->OnCollisionStay(this);
						}
						collided = true;
					}
				}
			}
		}
	}
	return collided;
}

bool HexagonCollider::CheckFloor2Sphere(SphereCollider* sphere, int f, float dt)
{
	GameObject* ob = objects[f];
	Sphere s = sphere->GetWorldSphere();
	Physics* physics = objects[f]->GetComponent<Physics>();
	float3 pastPos;
	GameObject* tg = GetGameObject();
	Transform* tgt = tg->GetTransform();
	float smallestT = 100;
	bool collided = false;

	if (physics)
	{
		pastPos = physics->pastPos + sphere->GetOffset();
	}

	//Top
	if (CheckSphereAABB(int_floor(row * 0.49f) * col, row * col - 1, s))
	{
		//TopTop
		if (CheckSphereAABB(int_floor(row * 0.741f) * col, row * col - 1, s))
		{
			if (CheckSphere(tg, ob, int_floor(row * 0.741f), row, sphere, s, pastPos, f, dt, smallestT))
				collided = true;
		}
		//TopBottom
		if (CheckSphereAABB(int_floor(row * 0.49f) * col, int_ceil(row * 0.759f) * col + col - 1, s))
		{
			if (CheckSphere(tg, ob, int_floor(row * 0.49f), int_ceil(row * 0.759f), sphere, s, pastPos, f, dt, smallestT))
				collided = true;
		}
	}
	//Bottom
	if (CheckSphereAABB(0, ceil(row * 0.5f) * col + col - 1, s))
	{
		//BottomTop
		if (CheckSphereAABB(int_floor(row * 0.241f) * col, int_ceil(row * 0.59f) * col + col - 1, s))
		{
			if (CheckSphere(tg, ob, int_floor(row * 0.241f), int_ceil(row * 0.59f), sphere, s, pastPos, f, dt, smallestT))
				collided = true;
		}
		//BottomBottom
		if (CheckSphereAABB(0, ceil(row * 0.259f) * col + col - 1, s))
		{
			if (CheckSphere(tg, ob, 0, ceil(row * 0.259f), sphere, s, pastPos, f, dt, smallestT))
				collided = true;
		}
	}
	if (!collided)
	{
		if (CollidingWith[f])
		{
			CollidingWith[f] = false;
			tg->OnCollisionExit(sphere);
			objects[f]->OnCollisionExit(this);
		}
		PulseFloorEvent pfe;
		pfe.floorIndex = -1;
		EventDispatcher::GetSingleton()->DispatchTo(&pfe, "Game");
	}
	return collided;
}

bool HexagonCollider::CheckSphere(GameObject* tg, GameObject* ob, int _min, int _max, SphereCollider* sphere, Sphere s, float3 pastPos, int f, float dt, float& St)
{
	bool collided = false;
	AABB test;
	AABB sphereBox;
	sphereBox.max = s.m_Center + s.m_Radius;
	sphereBox.min = s.m_Center - s.m_Radius;
	float smallT = St;
	for (int i = _min; i < _max; ++i)
	{
		test.min = poses[i * col] - float3(1, 10, 1);
		test.max = poses[i * col + col - 1] + float3(1, 20, 1);
		if (AABBtoAABB(sphereBox, test))
		{
			for (int j = 0; j < col; ++j)
			{
				float3 n = HexagonToSphere(*GetWorldHex(i * col + j), s, pastPos, smallT);
				if (!n.isEqual(zeroF) && smallT < St)
				{
					collisionNormal = n;
					sphere->collisionNormal = n;
					Physics* op = ob->GetComponent<Physics>();
					if (op)
					{
						St = smallT;
						op->HandlePhysics(ob->GetTransform(), ob->GetTransform()->GetVelocity(), s.m_Center - sphere->GetOffset(), true, n);
						if (!CollidingWith[f])
						{
							tg->OnCollisionEnter(sphere);
							ob->OnCollisionEnter(this);
							CollidingWith[f] = true;
							PulseFloorEvent pfe;
							pfe.floorIndex = i * col + j;
							EventDispatcher::GetSingleton()->DispatchTo(&pfe, "Game");
						}
						else if (CollidingWith[f])
						{
							tg->OnCollisionStay(sphere);
							ob->OnCollisionStay(this);
						}
						collided = true;
					}
				}
			}
		}
	}
	return collided;
}

void HexagonCollider::FixedUpdate(float dt)
{
	if (objects.size() == 0)
	{
		objects = *GetGameObject()->GetGameObjects();
		CollidingWith.resize(objects.size());
	}
	GameObject* tg = GetGameObject();
	Transform* tgt = tg->GetTransform();
	size_t size = objects.size();
	GameObject* ob;

	//If Not Floor
	if (!poses)
	{
		for (int i = 0; i < size; ++i)
		{
			ob = objects[i];
			bool c = false;
			for (int j = 0; j < ignore.size(); ++j)
			{
				if (ignore[j] == ob)
					c = true;
			}
			if (c) continue;
			SphereCollider* sphere = ob->GetComponent<SphereCollider>();
			if (sphere && !sphere->IgnoreHex())
			{
				if (!sphere->isTrigger() && sphere->isEnabled())
				{
					float3 pastPos = ob->GetComponent<Physics>()->pastPos;
					Sphere s = sphere->GetWorldSphere();
					float t = 100;
					float3 n = HexagonToSphere(*GetWorldHex(), s, pastPos, t, mesh);
					if (!n.isEqual(zeroF))
					{
						collisionNormal = n;
						Physics* op = ob->GetComponent<Physics>();
						if (op)
						{
							op->HandlePhysics(ob->GetTransform(), ob->GetTransform()->GetVelocity(), s.m_Center - sphere->GetOffset(), true, n);
							if (!CollidingWith[i])
							{
								tg->OnCollisionEnter(sphere);
								ob->OnCollisionEnter(this);
								CollidingWith[i] = true;
							}
							continue;
						}
					}
					if (CollidingWith[i])
					{
						tg->OnCollisionExit(sphere);
						ob->OnCollisionExit(this);
						CollidingWith[i] = false;
					}
				}
			}
			CapsuleCollider* cap = ob->GetComponent<CapsuleCollider>();
			if (cap && !cap->IgnoreHex())
			{
				if (!cap->isTrigger() && cap->isEnabled())
				{
					float3 pastPos = ob->GetComponent<Physics>()->pastPos + cap->GetCapsule().m_Segment.m_Start;
					float3 nvel = ob->GetTransform()->GetVelocity();
					Capsule c = cap->GetWorldCapsule();
					float t = 100;
					float3 n = HexagonToCapsule(*GetWorldHex(), c, pastPos, t, mesh);
					if (!n.isEqual(zeroF))
					{
						collisionNormal = n;
						bool apg = false;
						if (!n.isEqual(float3(0, 1, 0)))
							apg = true;
						Physics* op = ob->GetComponent<Physics>();
						if (op)
						{
							op->HandlePhysics(ob->GetTransform(), nvel, c.m_Segment.m_Start - cap->GetCapsule().m_Segment.m_Start, false, n, apg, true);
							if (!CollidingWith[i])
							{
								tg->OnCollisionEnter(cap);
								ob->OnCollisionEnter(this);
								CollidingWith[i] = true;
							}
							continue;
						}
					}
					if (CollidingWith[i])
					{
						tg->OnCollisionExit(cap);
						ob->OnCollisionExit(this);
						CollidingWith[i] = false;
					}
				}
			}
		}
	}
	//If Floor
	else
	{
		for (int f = 0; f < size; ++f)
		{
			ob = objects[f];
			bool c = false;
			for (int j = 0; j < ignore.size(); ++j)
			{
				if (ignore[j] == ob)
					c = true;
			}
			if (c) continue;
			bool collided = false;
			SphereCollider* sphere = ob->GetComponent<SphereCollider>();
			if (sphere && !sphere->IgnoreHex())
			{
				if (!sphere->isTrigger() && sphere->isEnabled())
				{
					CheckFloor2Sphere(sphere, f, dt);
				}
			}

			CapsuleCollider* cap = ob->GetComponent<CapsuleCollider>();
			if (cap && !cap->IgnoreHex())
			{
				if (!cap->isTrigger() && cap->isEnabled())
				{
					CheckFloor2Capsule(cap, f, dt);
				}
			}
		}
	}
}