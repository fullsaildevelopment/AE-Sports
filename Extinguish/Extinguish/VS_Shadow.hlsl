...
cbuffer LightMVPConstantBuffer : register(b2)
{
	matrix lightModel;
	matrix lightView;
...
PS_BasicInput main(VertexShaderInput input)
{
	...
	//calculate position as seen be light
	output.lightViewPosition  = mul(float4(input.pos, 0.0f), boneOffsets[input.blendingIndex.x]) * input.blendingWeight.x;
	output.lightViewPosition += mul(float4(input.pos, 0.0f), boneOffsets[input.blendingIndex.y]) * input.blendingWeight.y;
	output.lightViewPosition += mul(float4(input.pos, 0.0f), boneOffsets[input.blendingIndex.z]) * input.blendingWeight.z;
	output.lightViewPosition += mul(float4(input.pos, 0.0f), boneOffsets[input.blendingIndex.w]) * input.blendingWeight.w;

	output.lightViewPosition = mul(output.lightViewPosition, lightModel);
